...
| `Copper.getMod(Class)` | 通过主类所在的容器查找对应模组描述符 |
...
<Aside type="note" title="模组是通过类加载器找到的">
	`Copper.getMod(Class)` 用主类**所在的容器类加载器**反查模组，因此不再依赖包名。但 `main` 必须以 `id` 的点形式开头，这是[模组元数据](/developers/mod-meta/#main)本身的校验要求，仍需遵守。
</Aside>

详见[数据、设置与本地化](/developers/data/)中的使用示例，以及其中的[游戏内设置页面](/developers/data/#游戏内设置页面)。
