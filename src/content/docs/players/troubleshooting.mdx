...
**原因**：Copper 只收录带 Java 主类的模组。Copper 模组的元数据本来就要求 `main`，因此这种情况只会出现在**原版 Mindustry 模组**上：`mod.json` 未声明 `main`，或 `main` 指向的 `.class` 在 Jar 内不存在（见[管理模组 · 只有 Java 模组会被 Copper 收录](/players/manage-mods/#模组放在哪里)）。
...
**原因**：模组设置**不会**各自成为设置菜单的顶层条目，而是统一收在「设置 → 模组设置」页面里。

**解决**：进入「模组设置」，在页面顶部的搜索框输入模组名定位；对应模组没有出现在列表里，说明它没有提供设置分类（见[管理模组 · 游戏内模组设置](/players/manage-mods/#游戏内模组设置)）。
